...
test_ipc_security_descriptor(void)
{
	...
	NUTS_PASS(nng_listener_setopt_ptr(
	    l, NNG_OPT_IPC_SECURITY_DESCRIPTOR, sdesc));
	NUTS_PASS(nng_listener_start(l, 0));

	snprintf(pipe, sizeof(pipe, "\\\\.\\\\pipe\\%s", address)) HANDLE ph =
	    CreateFileA(pipe, READ_CONTROL, 0, NULL, OPEN_EXISTING,
	...
	NUTS_ASSERT(EqualSid(asid, &users) == TRUE);
...
test_ipc_security_descriptor_busy(void)
{
	...
	NUTS_FAIL(
	    nng_listener_setopt_ptr(l, NNG_OPT_IPC_SECURITY_DESCRIPTOR, sdesc),
	    NNG_EBUSY);
...
test_ipc_security_descriptor_bogus(void)
{
	...
	NUTS_FAIL(
	    nng_listener_setopt_ptr(l, NNG_OPT_IPC_SECURITY_DESCRIPTOR, NULL),
	    NNG_EINVAL);
...
test_ipc_security_descriptor_dialer(void)
{
	...
	InitializeSecurityDescriptor(sdesc, SECURITY_DESCRIPTOR_REVISION);
	NUTS_FAIL(
	    nng_dialer_setopt_ptr(d, NNG_OPT_IPC_SECURITY_DESCRIPTOR, sdesc),
	    NNG_ENOTSUP);
	free(sdesc);
